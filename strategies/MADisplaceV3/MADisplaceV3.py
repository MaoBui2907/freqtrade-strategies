...
class MADisplaceV3(IStrategy):
    ...
    ma_lower_offset = DecimalParameter(
        0.95, 0.97, default=buy_params["ma_lower_offset"], space="entry"
    )
    ...
    rsi_fast_threshold = IntParameter(
    ...
    ma_middle_1_length = IntParameter(
        15, 35, default=sell_params["ma_middle_1_length"], space="exit"
    )
    ma_middle_1_offset = DecimalParameter(
        0.93, 1.005, default=sell_params["ma_middle_1_offset"], space="exit"
    )
    ma_upper_length = IntParameter(15, 25, default=sell_params["ma_upper_length"], space="exit")
    ma_upper_offset = DecimalParameter(
        1.005, 1.025, default=sell_params["ma_upper_offset"], space="exit"
    )
    ...
    def get_informative_indicators(self, metadata: dict):
        ...
        dataframe["ema_fast"] = ta.EMA(
            dataframe, timeperiod=int(self.informative_fast_length.value)
        )
        dataframe["ema_slow"] = ta.EMA(
            dataframe, timeperiod=int(self.informative_slow_length.value)
        )
    ...
    def get_main_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ma_lower"] = (
            ta.SMA(dataframe, timeperiod=int(self.ma_lower_length.value))
            * self.ma_lower_offset.value
        )
        dataframe["ma_middle_1"] = (
            ta.SMA(dataframe, timeperiod=int(self.ma_middle_1_length.value))
            * self.ma_middle_1_offset.value
        )
        dataframe["ma_upper"] = (
            ta.SMA(dataframe, timeperiod=int(self.ma_upper_length.value))
            * self.ma_upper_offset.value
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (
                    dataframe["rsi_slow_descending"].rolling(self.rsi_slow_confirmation.value).sum()
