...
class Dyna_opti(IStrategy):
    ...
    inf_guard = CategoricalParameter(
        ["lower", "upper", "both", "none"], default="lower", space="entry", optimize=True
    )
    ...
    droi_pullback_respect_table = CategoricalParameter(
        [True, False], default=True, space="exit", optimize=True
    )

    # Custom Stoploss
    cstp_threshold = DecimalParameter(-0.15, 0, default=-0.05, space="exit")
    cstp_bail_how = CategoricalParameter(
        ["roc", "time", "any"], default="time", space="exit", optimize=True
    )
    ...
    # ROI table:
    minimal_roi = {"0": 0.18724, "20": 0.04751, "30": 0.02393, "72": 0}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp.runmode.value in ("backtest", "hyperopt"):
            ...
            self.custom_trade_info[metadata["pair"]]["rmi-slow"] = (
                dataframe[["date", "rmi-slow"]].copy().set_index("date")
            )
