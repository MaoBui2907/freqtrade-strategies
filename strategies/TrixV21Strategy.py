...
class TrixV21Strategy(IStrategy):
    ...
    buy_trix_signal_timeperiod = IntParameter(
        19, 25, default=22, space="buy", optimize=False, load=True
    )
    ...
    buy_ema_guard_timeperiod = IntParameter(
        150, 250, default=200, space="buy", optimize=True, load=True
    )
    buy_ema_guard_multiplier = DecimalParameter(
    ...
    sell_trix_signal_timeperiod = IntParameter(
        17, 23, default=19, space="sell", optimize=False, load=True
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            dataframe["close"]
            > (
                dataframe[f"ema_guard_{self.buy_ema_guard_timeperiod.value}"]
                * self.buy_ema_guard_multiplier.value
            )
        )
        conditions.append(
            dataframe["trix_s_pct"]
