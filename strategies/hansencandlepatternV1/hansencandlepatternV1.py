...
class hansencandlepatternV1(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["3LINESTRIKE"] = ta.CDL3LINESTRIKE(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["EVENINGSTAR"] = ta.CDLEVENINGSTAR(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["ABANDONEDBABY"] = ta.CDLEVENINGSTAR(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["HARAMI"] = ta.CDLHARAMI(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["INVERTEDHAMMER"] = ta.CDLINVERTEDHAMMER(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["ENGULFING"] = ta.CDLENGULFING(
            dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"]
        )
        dataframe["hclose"] = (
            dataframe["open"] + dataframe["high"] + dataframe["low"] + dataframe["close"]
