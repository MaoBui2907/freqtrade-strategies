...
class MacheteV8bRallimod2(IStrategy):
    ...
    buy_should_use_get_buy_signal_offset_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_offset_strategy"],
        space="entry",
        optimize=True,
    )
    buy_should_use_get_buy_signal_bbrsi_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_bbrsi_strategy"],
        space="entry",
        optimize=True,
    )
    ...
    droi_pullback = CategoricalParameter(
        [True, False], default=sell_params["droi_pullback"], space="exit", optimize=True
    )
    droi_pullback_amount = DecimalParameter(
        0.005, 0.02, default=sell_params["droi_pullback_amount"], space="exit"
    )
    ...
    cstp_bail_how = CategoricalParameter(
        ["roc", "time", "any"], default=sell_params["cstp_bail_how"], space="exit", optimize=True
    )
    cstp_bail_roc = DecimalParameter(
        -0.05, -0.01, default=sell_params["cstp_bail_roc"], space="exit"
    )
    ...
    cstp_trailing_only_offset_is_reached = DecimalParameter(
        0.01, 0.06, default=sell_params["cstp_trailing_only_offset_is_reached"], space="exit"
    )
    cstp_trailing_stop_profit_devider = IntParameter(
        2, 4, default=sell_params["cstp_trailing_stop_profit_devider"], space="exit"
    )
    cstp_trailing_max_stoploss = DecimalParameter(
        0.02, 0.08, default=sell_params["cstp_trailing_max_stoploss"], space="exit"
    )
