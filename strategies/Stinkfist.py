...
class Stinkfist(IStrategy):
    ...
    buy_params = {"inf-pct-adr": 0.80, "mp": 30}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data["active_trade"]:
            ...
            conditions.append(
                trade_data["current_profit"] > (trade_data["peak_profit"] * profit_factor)
            )
            ...
            conditions.append(
                ...
                (
                    ...
                    + (params["inf-pct-adr"] * dataframe[f"adr_{self.inf_timeframe}"])
                )
                &
    ...
    def populate_trades(self, pair: str) -> dict:
        ...
        trade_data = {}
        trade_data["active_trade"] = trade_data["other_trades"] = trade_data["biggest_loser"] = (
            False
        )
        ...
        if self.config["runmode"].value in ("live", "dry_run"):
            ...
            if active_trade:
                ...
                trade_data["peak_profit"] = max(
                    0, active_trade[0].calc_profit_ratio(active_trade[0].max_rate)
                )
            ...
            if other_trades:
                ...
                other_profit = tuple(
                    trade.calc_profit_ratio(self.get_current_price(trade.pair, False))
    ...
    def linear_growth(
        self, start: float, end: float, start_time: int, end_time: int, trade_time: int
    ) -> float:
    ...
    def mac(self, dataframe: DataFrame, fast: int = 20, slow: int = 50) -> Series:
        ...
        # Crosses
        crosses_lf_us = qtpylib.crossed_above(lower_fast, upper_slow) | qtpylib.crossed_below(
            lower_fast, upper_slow
        )
        crosses_uf_ls = qtpylib.crossed_above(upper_fast, lower_slow) | qtpylib.crossed_below(
    ...
    def pcc(self, dataframe: DataFrame, period: int = 20, mult: int = 2):
        PercentChangeChannel = namedtuple(
            "PercentChangeChannel", ["upperband", "middleband", "lowerband"]
        )
