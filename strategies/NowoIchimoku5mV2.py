...
def merge_informative_pair(
    ...
    dataframe = pd.merge(
        dataframe, informative, left_on="date", right_on=f"date_merge_{timeframe_inf}", how="left"
    )
...
class NowoIchimoku5mV2(IStrategy):
    ...
    plot_config = {
        ...
        "subplots": {
            "Buy Allowed": {
                "buy_allowed": {"color": "blue"},
            },
            "Should Buy": {
                "should_buy": {"color": "green"},
            },
    ...
    close_above_shifted_upper_cloud = DecimalParameter(
        0.5, 2, decimals=3, default=0.603, space="buy"
    )
    ...
    def populate_indicators(self, df_5m: DataFrame, metadata: dict) -> DataFrame:
        ...
        df_1h["upper"] = bollinger_bands(df_1h["close"], moving_average="hma", length=20, mult=2.5)[
            "upper"
        ]
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        close_above_shifted_upper_cloud = (
            df["close"] > df["shifted_upper_cloud"] * self.close_above_shifted_upper_cloud.value
        )
        ...
        for i in range(1, len(df)):
            ...
            df.loc[i, "entry"] = df.at[i, "buy_allowed"] & df.at[i, "should_buy"]
