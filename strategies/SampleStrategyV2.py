...
class SampleStrategyV2(IStrategy):
    ...
    buy_trend_length = IntParameter(
        low=50, high=288, default=288, space="entry", optimize=True, load=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ha_open_sma288"] = ta.SMA(
            dataframe["ha_open"], timeperiod=self.buy_trend_length.value
        )
        dataframe["ha_close_sma288"] = ta.SMA(
            dataframe["ha_close"], timeperiod=self.buy_trend_length.value
        )
        dataframe["trend_dir"] = ta.SMA(
            ta.SMA(dataframe["ha_close_sma288"] - dataframe["ha_open_sma288"], timeperiod=5),
            timeperiod=5,
        )
