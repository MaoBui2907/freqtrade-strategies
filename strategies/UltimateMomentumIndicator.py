...
class UltimateMomentumIndicator(IStrategy):
    ...
    length1_buy = IntParameter(2, 20, default=13, space="entry")
    length2_buy = IntParameter(10, 40, default=19, space="entry")
    length3_buy = IntParameter(10, 50, default=21, space="entry")
    length4_buy = IntParameter(20, 80, default=39, space="entry")
    length5_buy = IntParameter(30, 100, default=50, space="entry")
    length6_buy = IntParameter(150, 300, default=200, space="entry")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["bPct"] = np.where(
            ...
            (dataframe[source] - dataframe["lowerBand"])
            / (dataframe["upperBand"] - dataframe["lowerBand"]),
            0,
        )

        dataframe["advSum"] = (
            pd.Series(np.where(dataframe[source].diff() > 0, 1, 0)).rolling(length2).sum()
        )
        dataframe["decSum"] = (
            pd.Series(np.where(dataframe[source].diff() > 0, 0, 1)).rolling(length2).sum()
        )
        dataframe["ratio"] = np.where(
            dataframe["decSum"] != 0, dataframe["advSum"] / dataframe["decSum"], 0
        )
        ...
        dataframe["mo"] = ta.EMA(dataframe["rana"], timeperiod=length2) - ta.EMA(
            dataframe["rana"], timeperiod=length4
        )
        dataframe["utm"] = (
            (200 * dataframe["bPct"])
            + (100 * dataframe["ratio"])
