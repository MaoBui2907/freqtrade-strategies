...
class EI3v2_tag_cofi_green(IStrategy):
    ...
    base_nb_candles_buy = IntParameter(
        8, 20, default=buy_params["base_nb_candles_buy"], space="entry", optimize=False
    )
    base_nb_candles_sell = IntParameter(
        8, 20, default=sell_params["base_nb_candles_sell"], space="exit", optimize=False
    )
    low_offset = DecimalParameter(
        0.985, 0.995, default=buy_params["low_offset"], space="entry", optimize=True
    )
    high_offset = DecimalParameter(
        1.005, 1.015, default=sell_params["high_offset"], space="exit", optimize=True
    )
    high_offset_2 = DecimalParameter(
        1.010, 1.020, default=sell_params["high_offset_2"], space="exit", optimize=True
    )
    ...
    ewo_high = DecimalParameter(
        3.0, 3.4, default=buy_params["ewo_high"], space="entry", optimize=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        drop_columns = [
            f"{s}_{self.inf_1h}" for s in ["date", "open", "high", "low", "close", "volume"]
        ]
...
class EI3v2_tag_cofi_dca_green(EI3v2_tag_cofi_green):
    ...
    dca_min_rsi = IntParameter(
        35, 75, default=buy_params["dca_min_rsi"], space="entry", optimize=True
    )
