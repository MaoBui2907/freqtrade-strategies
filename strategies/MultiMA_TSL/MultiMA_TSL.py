...
class MultiMA_TSL(IStrategy):
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ema_offset_sell"] = (
            ta.EMA(dataframe, int(self.base_nb_candles_sell.value)) * self.high_offset_ema.value
        )
        dataframe["trima_offset_sell"] = (
            ta.TRIMA(dataframe, int(self.base_nb_candles_sell_trima.value))
            * self.high_offset_trima.value
        )

        conditions = []

        conditions.append(
            ((dataframe["close"] > dataframe["ema_offset_sell"]) & (dataframe["volume"] > 0))
        )

        conditions.append(
            ((dataframe["close"] > dataframe["trima_offset_sell"]) & (dataframe["volume"] > 0))
        )

        if conditions:
            dataframe.loc[reduce(lambda x, y: x | y, conditions), ["sell_copy", "exit_long"]] = (
                1,
                1,
            )
