...
class CombinedBinHAndClucHyperV0(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for x in (
            self.sell_bb_middleband_window.range
            if isinstance(self.sell_bb_middleband_window, ABC)
            else [self.sell_bb_middleband_window]
        ):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        sell_bb_middleband_window = (
            self.sell_bb_middleband_window.value
            if isinstance(self.sell_bb_middleband_window, ABC)
            else self.sell_bb_middleband_window
        )
        ...
        dataframe.loc[
            (  # strategy BinHV45
                ...
                & dataframe[f"bb_middleband_{sell_bb_middleband_window}"].gt(
                    dataframe["close"] * buy_a_min_sell_rate
                )
            )
            | (  # strategy ClucMay72018
                ...
                & (
                    dataframe["close"]
                    < buy_b_close_rate * dataframe[f"bb_lowerband_{buy_b_time_window}"]
                )
                & (
                    dataframe["volume"]
                    < (
