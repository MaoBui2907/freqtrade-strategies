...
class BB_RPB_TSL_SMA_Tranz_TB_MOD(IStrategy):
    ...
    ewo_low2 = DecimalParameter(
        -20.0, -8.0, default=buy_params["ewo_low2"], space="buy", optimize=True
    )
    ewo_high2 = DecimalParameter(
        2.0, 12.0, default=buy_params["ewo_high2"], space="buy", optimize=True
    )
    ...
    def pump_warning2(self, dataframe):
        ...
        df["pct_change"] = df["close"].pct_change(
            periods=self.pump_protection_01_pct_change_timeframe.value
        )
        df["pct_change_int"] = (
            df["pct_change"] > self.pump_protection_01_pct_change_max.value
        ).astype(int) | (df["pct_change"] < self.pump_protection_01_pct_change_min.value).astype(
            int
        )
        df["pct_change_short"] = df["close"].pct_change(
            periods=self.pump_protection_01_pct_change_short_timeframe.value
        )
        df["pct_change_int_short"] = (
            df["pct_change_short"] > self.pump_protection_01_pct_change_short_max.value
        ).astype(int) | (
            df["pct_change_short"] < self.pump_protection_01_pct_change_short_min.value
        ).astype(int)
        df["ispumping"] = (
            df["pct_change_int"].rolling(20).sum() >= self.pump_protection_01_ispumping.value
        ).astype("int")
        df["islongpumping"] = (
            df["pct_change_int"].rolling(30).sum() >= self.pump_protection_01_islongpumping.value
        ).astype("int")
        df["isshortpumping"] = (
            df["pct_change_int_short"].rolling(10).sum()
            >= self.pump_protection_01_isshortpumping.value
        ).astype("int")
        df["recentispumping"] = (
            (df["ispumping"].rolling(300).max() > 0)
            | (df["islongpumping"].rolling(300).max() > 0)
            | (df["isshortpumping"].rolling(300).max() > 0)
        )
    ...
    def informative_15m_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_15m = self.dp.get_pair_dataframe(
            pair=metadata["pair"], timeframe=self.info_timeframe_15m
        )
                    ...
                    if trailing_buy["allow_trailing"]:
                        if not trailing_buy["trailing_buy_order_started"] and (
                            ...
                            if trailing_buy_offset == "forcebuy":
                                ...
                                val = True
                                ratio = "%.2f" % (
                                    (self.current_trailing_profit_ratio(pair, current_price)) * 100
                                )
                                self.trailing_buy_info(pair, current_price)
                                log.info(
