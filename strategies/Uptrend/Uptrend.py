...
class Uptrend(IStrategy):
    ...
    buy_rsi_uplimit = IntParameter(
        50, 90, default=buy_params["buy_rsi_uplimit"], optimize=False, space="entry"
    )
...
class SuperBuy(Uptrend):
    ...
    generator = IntParameter(
        0, 100000000000, default=99295874569, optimize=True, space="entry"
    )  # generate unique matrix of conditions for your dataframe
    operators_used_to_compare_between_columns = IntParameter(
        0, 3, default=3, optimize=True, space="entry"
    )  # number of conditions you will keep to build buy signal
    operators_used_to_with_best_point = IntParameter(
        0, 3, default=1, optimize=True, space="entry"
    )  # number of conditions you will keep to build buy signal
    condition_selector = IntParameter(
        0, 100, default=50, optimize=True, space="entry"
    )  # how to select the desired conditions beteween all conditions generated (seed random)
    ...
    def find_best_entry_point(self, dataframe: DataFrame, metadata: dict):
        ...
        workdataframe["higher_high_close_ratio"] = (
            workdataframe["higher_high"] / workdataframe["close_shifted_lookehead"]
        )

        df_mask = (
            workdataframe["higher_high_close_ratio"]
            >= 1 + self.top_index_criteria["min_close_hh_ratio"]
        )
        ...
        filtered_df["shifted_index"] = (
            filtered_df.index
            - lookahead_candles
            + self.top_index_criteria["candles_after_dip_to_buy"]
        )
    ...
    def common_points_for_every_best_entry(
        self, dataframe: DataFrame, metadata: dict, columns: list
    ) -> list:
        full_pairlist = self.dp.current_whitelist()
        current_pair = metadata["pair"]

        if current_pair not in self.best_buy_point_dict:
            self.best_buy_point_dict[current_pair], self.bad_buy_point_dict[current_pair] = (
                self.find_best_entry_point(dataframe, metadata)
            )
        ...
        for column in columns:
            ...
            if not count.empty and column not in ["entry", "buy_tag"]:
                ...
                all_bad_points_values_count_normalized = (
                    all_bad_points_values_count / all_bad_points.shape[0]
                )
                df_all = count.to_frame(name="best_points").join(
                    all_bad_points_values_count.to_frame(name="bad_points")
                )
                df_all["best_point_percent"] = count_normalized
                df_all["bad_point_percent"] = all_bad_points_values_count_normalized
                df_all["part_of_best_points"] = (
                    100 * df_all["best_points"] / (df_all["best_points"] + df_all["bad_points"])
                )
                df_all["part_of_best_points_percent"] = (
                    count_normalized / all_bad_points_values_count_normalized
                )
                print(column)
                print(df_all)
                values = df_all.query(
                ...
                res.append(
                    {
                        ...
                        "ratio_for_all": all_points_values_count[count.index[0]]
                        / all_points.shape[0],
                        "ratio_diff": count.iloc[0] / all_best_points.shape[0]
                        - all_points_values_count[count.index[0]] / all_points.shape[0],
                    }
                )
        for item in sorted(res, key=lambda x: x["ratio_diff"]):
            print(
                f"({item['column']} == {item['value']}), {100 * item['ratio_for_best']:.2f}% in best vs {100 * item['ratio_for_all']:.2f}% average"
            )
        ...
        for item in sorted(res, key=lambda x: x["ratio_diff"]):
            if (
                ((100 * item["ratio_for_best"]) - (100 * item["ratio_for_all"]))
                > self.top_index_criteria[
                    "select_prameter_if_prop_is_x_percent_higher_in_best_points"
                ]
            ) and (100 * item["ratio_for_best"]) > self.top_index_criteria[
                "select_parameter_if_in_more_than_x_percent_of_best_points"
            ]:
    ...
    def is_same_dimension_as_price(self, dataframe: DataFrame, column_name: str) -> bool:
        if dataframe["close"].dtype != dataframe[column_name].dtype:
            ...
            return False
        return (
            dataframe[column_name].max() <= dataframe["high"].max()
            and dataframe[column_name].min() >= dataframe["low"].min()
        )
    ...
    def generate_superbuy_signal(self, dataframe: DataFrame, metadata: dict) -> list:
        ...
        if self.best_buy_point is None:
            try:
                top_index, _ = self.find_best_entry_point(dataframe, metadata)[
                    "shifted_index"
                ].iloc[0]
        ...
        if (
            len(self.columns_to_compare_to_best_point) == 0
            and len(self.columns_to_compare_to_volume) == 0
            and len(self.columns_to_compare_to_price) == 0
        ):
        ...
        for column in self.columns_to_compare_to_best_point:
            ...
            buy_conds_best_point.append(
                ...
                + self.operators[int(generator)]
                + " "
                + str(self.best_buy_point[column])
                + ")"
            )
        ...
        try:
            buy_conds = random.sample(
                buy_conds, self.operators_used_to_compare_between_columns.value
            )
        except ValueError:
        ...
        try:
            buy_conds += random.sample(
                buy_conds_best_point, self.operators_used_to_with_best_point.value
            )
        except ValueError:
            if self.config["runmode"].value != "hyperopt":
            ...
            pass
        if (
            self.config["runmode"].value in ("backtest", "hyperopt")
            and self.buy_signal_already_printed != buy_conds
        ):
            print(buy_conds)
        ...
        try:
            buy_conds = [
                eval(
                    buy_cond,
                    globals(),
                    {"dataframe": dataframe, "best_buy_point": self.best_buy_point},
                )
                for buy_cond in buy_conds
            ]
        except:
            return []
        return buy_conds
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.config["runmode"].value in (
            "backtest"
        ):  # backtest, we want to check must common buy tags...
            dataframe = super().populate_entry_trend(dataframe, metadata)  # get buy tags
            self.common_points_for_every_best_entry(
                dataframe, metadata, self.columns + ["entry", "buy_tag"]
            )
        elif self.config["runmode"].value in (
            "hyperopt"
        ):  # hyperopt, we want to test new buy signals
            is_additional_check = (
            ...
            if buy_conds:
                dataframe.loc[
                    is_additional_check & reduce(lambda x, y: x & y, buy_conds), "enter_long"
                ] = 1
