...
class CombinedBinHAndClucHyperV3(IStrategy):
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        sell_trailing_stop_positive_offset = (
            self.sell_trailing_stop_positive_offset.value
            if isinstance(self.sell_trailing_stop_positive_offset, ABC)
            else self.sell_trailing_stop_positive_offset
        )
        sell_trailing_stop_positive = (
            self.sell_trailing_stop_positive.value
            if isinstance(self.sell_trailing_stop_positive, ABC)
            else self.sell_trailing_stop_positive
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # strategy BinHV45
        for x in (
            self.buy_a_time_window.range
            if isinstance(self.buy_a_time_window, ABC)
            else [self.buy_a_time_window]
        ):
        ...
        for x in (
            self.buy_b_time_window.range
            if isinstance(self.buy_b_time_window, ABC)
            else [self.buy_b_time_window]
        ):
        ...
        for x in (
            self.buy_b_ema_slow.range
            if isinstance(self.buy_b_ema_slow, ABC)
            else [self.buy_b_ema_slow]
        ):
            dataframe[f"ema_slow_{x}"] = ta.EMA(dataframe, timeperiod=x)

        for x in (
            self.buy_b_volume_mean_slow_window.range
            if isinstance(self.buy_b_volume_mean_slow_window, ABC)
            else [self.buy_b_volume_mean_slow_window]
        ):
            dataframe[f"volume_mean_slow_{x}"] = dataframe["volume"].rolling(window=x).mean()

        for x in (
            self.sell_bb_mid_slow_window.range
            if isinstance(self.sell_bb_mid_slow_window, ABC)
            else [self.sell_bb_mid_slow_window]
        ):
            sell_bollinger = qtpylib.bollinger_bands(
                qtpylib.typical_price(dataframe), window=x, stds=2
            )
            dataframe[f"bb_typical_mid_{x}"] = sell_bollinger["mid"]

        for x in (
            self.buy_a_atr_window.range
            if isinstance(self.buy_a_atr_window, ABC)
            else [self.buy_a_atr_window]
        ):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        buy_a_time_window = (
            self.buy_a_time_window.value
            if isinstance(self.buy_a_time_window, ABC)
            else self.buy_a_time_window
        )
        buy_a_bbdelta_rate = (
            self.buy_a_bbdelta_rate.value
            if isinstance(self.buy_a_bbdelta_rate, ABC)
            else self.buy_a_bbdelta_rate
        )
        buy_a_closedelta_rate = (
            self.buy_a_closedelta_rate.value
            if isinstance(self.buy_a_closedelta_rate, ABC)
            else self.buy_a_closedelta_rate
        )
        buy_a_tail_rate = (
            self.buy_a_tail_rate.value
            if isinstance(self.buy_a_tail_rate, ABC)
            else self.buy_a_tail_rate
        )
        buy_a_min_sell_rate = (
            self.buy_a_min_sell_rate.value
            if isinstance(self.buy_a_min_sell_rate, ABC)
            else self.buy_a_min_sell_rate
        )
        buy_a_atr_rate = (
            self.buy_a_atr_rate.value
            if isinstance(self.buy_a_atr_rate, ABC)
            else self.buy_a_atr_rate
        )
        buy_a_atr_window = (
            self.buy_a_atr_window.value
            if isinstance(self.buy_a_atr_window, ABC)
            else self.buy_a_atr_window
        )

        buy_b_ema_slow = (
            self.buy_b_ema_slow.value
            if isinstance(self.buy_b_ema_slow, ABC)
            else self.buy_b_ema_slow
        )
        buy_b_close_rate = (
            self.buy_b_close_rate.value
            if isinstance(self.buy_b_close_rate, ABC)
            else self.buy_b_close_rate
        )
        buy_b_time_window = (
            self.buy_b_time_window.value
            if isinstance(self.buy_b_time_window, ABC)
            else self.buy_b_time_window
        )
        buy_b_volume_mean_slow_window = (
            self.buy_b_volume_mean_slow_window.value
            if isinstance(self.buy_b_volume_mean_slow_window, ABC)
            else self.buy_b_volume_mean_slow_window
        )
        buy_b_volume_mean_slow_num = (
            self.buy_b_volume_mean_slow_num.value
            if isinstance(self.buy_b_volume_mean_slow_num, ABC)
            else self.buy_b_volume_mean_slow_num
        )

        sell_bb_mid_slow_window = (
            self.sell_bb_mid_slow_window.value
            if isinstance(self.sell_bb_mid_slow_window, ABC)
            else self.sell_bb_mid_slow_window
        )
        ...
        dataframe.loc[
            (  # strategy BinHV45
                ...
                & dataframe[f"bbdelta_{buy_a_time_window}"].gt(
                    dataframe["close"] * buy_a_bbdelta_rate
                )
                & dataframe[f"closedelta_{buy_a_time_window}"].gt(
                    dataframe["close"] * buy_a_closedelta_rate
                )
                & dataframe[f"tail_{buy_a_time_window}"].lt(
                    dataframe[f"bbdelta_{buy_a_time_window}"] * buy_a_tail_rate
                )
                ...
                & dataframe[f"bb_typical_mid_{sell_bb_mid_slow_window}"].gt(
                    ...
                    * (
                        buy_a_min_sell_rate
                        + dataframe[f"atr_rate_{buy_a_atr_window}"] * buy_a_atr_rate
                    )
                )
            )
            | (  # strategy ClucMay72018
                ...
                & (
                    dataframe["close"]
                    < buy_b_close_rate * dataframe[f"bb_typical_lower_{buy_b_time_window}"]
                )
                & (
                    dataframe["volume"]
                    < (
                        dataframe[f"volume_mean_slow_{buy_b_volume_mean_slow_window}"].shift(1)
                        * buy_b_volume_mean_slow_num
                    )
                )
            ),
