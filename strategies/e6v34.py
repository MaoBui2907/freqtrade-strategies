...
class e6v34(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["willr"] = ta.WILLR(
            dataframe["high"], dataframe["low"], dataframe["close"], timeperiod=pwill
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(
            dataframe[f"hma{shma}"].shift(1) - dataframe[f"hma{lhma}"].shift(1)
            < dataframe[f"hma{shma}"] - dataframe[f"hma{lhma}"]
        )
        conditions.append(
            dataframe[f"hma{shma}"].shift(2) - dataframe[f"hma{lhma}"].shift(2)
            < dataframe[f"hma{shma}"].shift(1) - dataframe[f"hma{lhma}"].shift(1)
        )
