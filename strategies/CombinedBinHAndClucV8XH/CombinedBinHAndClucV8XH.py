...
class CombinedBinHAndClucV8XH(IStrategy):
    ...
    buy_dip_threshold_0 = DecimalParameter(
        0.001, 0.1, default=0.015, space='entry', decimals=3, optimize=False, load=True)
    buy_dip_threshold_1 = DecimalParameter(
        0.08, 0.2, default=0.12, space='entry', decimals=2, optimize=False, load=True)
    buy_dip_threshold_2 = DecimalParameter(
        0.02, 0.4, default=0.28, space='entry', decimals=2, optimize=False, load=True)
    buy_dip_threshold_3 = DecimalParameter(
        0.25, 0.44, default=0.36, space='entry', decimals=2, optimize=False, load=True)

    buy_bb40_bbdelta_close = DecimalParameter(
        0.005, 0.04, default=0.031, space='entry', optimize=True, load=True)
    buy_bb40_closedelta_close = DecimalParameter(
        0.01, 0.03, default=0.021, space='entry', optimize=True, load=True)
    buy_bb40_tail_bbdelta = DecimalParameter(
        0.2, 0.4, default=0.264, space='entry', optimize=True, load=True)

    buy_bb20_close_bblowerband = DecimalParameter(
        0.8, 1.1, default=0.992, space='entry', optimize=True, load=True)
    buy_bb20_volume = IntParameter(
        18, 36, default=29, space='entry', optimize=True, load=True)

    buy_rsi_diff = DecimalParameter(
        34.0, 60.0, default=50.48, space='entry', decimals=2, optimize=True, load=True)

    buy_min_inc = DecimalParameter(
        0.005, 0.05, default=0.01, space='entry', decimals=2, optimize=True, load=True)
    buy_rsi_1h = DecimalParameter(
        40.0, 70.0, default=67.0, space='entry', decimals=2, optimize=True, load=True)
    buy_rsi = DecimalParameter(
        30.0, 40.0, default=38.5, space='entry', decimals=2, optimize=True, load=True)
    buy_mfi = DecimalParameter(
        36.0, 65.0, default=36.0, space='entry', decimals=2, optimize=True, load=True)

    buy_volume_1 = DecimalParameter(
        1.0, 10.0, default=2.0, space='entry', decimals=2, optimize=False, load=True)
    buy_ema_open_mult_1 = DecimalParameter(
        0.01, 0.05, default=0.02, space='entry', decimals=3, optimize=False, load=True)

    # Sell Hyperopt params

    sell_custom_roi_profit_1 = DecimalParameter(
        0.01, 0.03, default=0.01, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_rsi_1 = DecimalParameter(
        40.0, 56.0, default=50, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_profit_2 = DecimalParameter(
        0.01, 0.20, default=0.04, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_rsi_2 = DecimalParameter(
        42.0, 56.0, default=50, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_profit_3 = DecimalParameter(
        0.15, 0.30, default=0.08, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_rsi_3 = DecimalParameter(
        44.0, 58.0, default=56, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_profit_4 = DecimalParameter(
        0.3, 0.7, default=0.14, space='exit', decimals=2, optimize=False, load=True)
    sell_custom_roi_rsi_4 = DecimalParameter(
        44.0, 60.0, default=58, space='exit', decimals=2, optimize=False, load=True)

    sell_custom_roi_profit_5 = DecimalParameter(
        0.01, 0.1, default=0.04, space='exit', decimals=2, optimize=False, load=True)

    sell_trail_profit_min_1 = DecimalParameter(
        0.1, 0.25, default=0.1, space='exit', decimals=3, optimize=False, load=True)
    ...
    sell_trail_down_1 = DecimalParameter(
        0.04, 0.1, default=0.03, space='exit', decimals=3, optimize=False, load=True)
    ...
    sell_trail_down_2 = DecimalParameter(
        0.04, 0.2, default=0.015, space='exit', decimals=3, optimize=False, load=True)

    sell_custom_stoploss_1 = DecimalParameter(
        -0.15, -0.03, default=-0.05, space='exit', decimals=2, optimize=False, load=True)

    sell_rsi_main = DecimalParameter(
        72.0, 90.0, default=80, space='exit', decimals=2, optimize=True, load=True)

    #X version additional RSI value
    sell_rsi_parachute = DecimalParameter(
        30.0, 55.0, default=40, space='exit', decimals=2, optimize=True, load=True)
