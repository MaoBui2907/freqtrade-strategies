...
class PRICEFOLLOWINGX(IStrategy):
    ...
    ema_pct = DecimalParameter(
        0.001, 0.100, decimals=3, default=0.040, space="buy", optimize=True, load=True
    )
    buy_frsi = DecimalParameter(
        -0.71, 0.50, decimals=2, default=-0.40, space="buy", optimize=True, load=True
    )
    frsi_pct = DecimalParameter(
        0.01, 0.20, decimals=2, default=0.10, space="buy", optimize=True, load=True
    )
    # sellspace
    ema_sell_pct = DecimalParameter(
        0.001, 0.020, decimals=3, default=0.003, space="sell", optimize=True, load=True
    )
    sell_rsi_enabled = BooleanParameter(default=True, space="exit", optimize=True, load=True)
