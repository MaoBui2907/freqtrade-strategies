...
from freqtrade.strategy.hyper import CategoricalParameter, IntParameter, RealParameter
...
class MarketChyperHyperStrategy(IStrategy):
    ...
    buy___trades_when_downwards = CategoricalParameter(
        [True, False], default=True, space="entry", optimize=False, load=True
    )
    buy___trades_when_sideways = CategoricalParameter(
        [True, False], default=True, space="entry", optimize=False, load=True
    )
    buy___trades_when_upwards = CategoricalParameter(
        [True, False], default=True, space="entry", optimize=False, load=True
    )

    buy_uptrend_total_signal_needed = IntParameter(
        0, 100, default=65, space="entry", optimize=True, load=True
    )
    # wave trend
    buy_uptrend_wavetrend_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )
    # rsi divs
    buy_uptrend_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    # buy_rsi_div_value (rsi used in condition of bullish div)
    buy_uptrend_rsi_div_value = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    buy_uptrend_oslevel = IntParameter(-100, 0, default=0, space="entry", optimize=True, load=True)

    # Sideways Trend Buy
    # -------------------
    buy_sideways_total_signal_needed = IntParameter(
        0, 100, default=65, space="entry", optimize=True, load=True
    )
    # wave trend
    buy_sideways_wavetrend_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )
    # rsi divs
    buy_sideways_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    # buy_rsi_div_value (rsi used in condition of bullish div)
    buy_sideways_rsi_div_value = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    buy_sideways_oslevel = IntParameter(-100, 0, default=0, space="entry", optimize=True, load=True)

    # Downtrend Trend Buy
    # -------------------
    buy_downtrend_total_signal_needed = IntParameter(
        0, 100, default=65, space="entry", optimize=True, load=True
    )
    # wave trend
    buy_downtrend_wavetrend_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )
    # rsi divs
    buy_downtrend_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    # buy_rsi_div_value (rsi used in condition of bullish div)
    buy_downtrend_rsi_div_value = IntParameter(
        0, 100, default=0, space="entry", optimize=True, load=True
    )

    buy_downtrend_oslevel = IntParameter(
        -100, 0, default=0, space="entry", optimize=True, load=True
    )

    # sell??
    sell___trades_when_downwards = CategoricalParameter(
        [True, False], default=True, space="exit", optimize=True, load=True
    )
    sell___trades_when_sideways = CategoricalParameter(
        [True, False], default=True, space="exit", optimize=True, load=True
    )
    sell___trades_when_upwards = CategoricalParameter(
        [True, False], default=True, space="exit", optimize=True, load=True
    )
    ...
    sell_uptrend_total_signal_needed = IntParameter(
        0, 100, default=65, space="exit", optimize=True, load=True
    )
    # wave trend
    sell_uptrend_wavetrend_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )
    # rsi divs
    sell_uptrend_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )

    sell_uptrend_oblevel = RealParameter(0, 100, default=0, space="exit", optimize=True, load=True)

    # Sidewyas Trend Sell
    # --------------------

    # Total Sell Signal Percentage needed for a signal to be positive
    sell_sideways_total_signal_needed = IntParameter(
        0, 100, default=65, space="exit", optimize=True, load=True
    )
    # wave trend
    sell_sideways_wavetrend_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )
    # rsi divs
    sell_sideways_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )

    sell_sideways_oblevel = RealParameter(0, 100, default=0, space="exit", optimize=True, load=True)

    # Downtrend Trend Sell
    # --------------------
    sell_downtrend_total_signal_needed = IntParameter(
        0, 100, default=65, space="exit", optimize=True, load=True
    )
    # wave trend
    sell_downtrend_wavetrend_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )
    # rsi divs
    sell_downtrend_rsi_divergence_weight = IntParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )

    sell_downtrend_oblevel = RealParameter(
        0, 100, default=0, space="exit", optimize=True, load=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        )  # 14 timeperiods is usually used for ADX
        ...
        dataframe["macdsignal"] = macd[
            "macdsignal"
        ]  # Signal - Orange TradingView Line (Bearish if on top)
        ...
        # get trend
        dataframe.loc[
            (dataframe["adx"] > 20) & (dataframe["plus_di"] < dataframe["minus_di"]), "trend"
        ] = "downwards"
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe["trend"] == "downwards"), "wtOversold"] = (
            dataframe["wt2"] <= self.buy_downtrend_oslevel.value
        )
        dataframe.loc[(dataframe["trend"] == "sideways"), "wtOversold"] = (
            dataframe["wt2"] <= self.buy_sideways_oslevel.value
        )
        dataframe.loc[(dataframe["trend"] == "upwards"), "wtOversold"] = (
            dataframe["wt2"] <= self.buy_uptrend_oslevel.value
        )
        ...
        ] = self.buy_downtrend_wavetrend_weight.value
        dataframe.loc[
            (dataframe["trend"] == "sideways") & (dataframe["wtCrossUp"] & dataframe["wtOversold"]),
            "buy_wavetrend_weight",
        ] = self.buy_sideways_wavetrend_weight.value
        dataframe.loc[
            (dataframe["trend"] == "upwards") & (dataframe["wtCrossUp"] & dataframe["wtOversold"]),
            "buy_wavetrend_weight",
        ] = self.buy_uptrend_wavetrend_weight.value
        ...
        ] = self.buy_downtrend_rsi_divergence_weight.value
        dataframe.loc[
            ...
            "buy_rsi_divergence_weight",
        ] = self.buy_sideways_rsi_divergence_weight.value
        dataframe.loc[
            ...
            "buy_rsi_divergence_weight",
        ] = self.buy_uptrend_rsi_divergence_weight.value
        ...
        dataframe.loc[
            (
                ...
                & (
                    ...
                    >= self.buy_downtrend_total_signal_needed.value
                )
            )
            | (
                (dataframe["trend"] == "sideways")
                & (
                    dataframe["total_buy_signal_strength"]
                    >= self.buy_sideways_total_signal_needed.value
                )
            )
            | (
                (dataframe["trend"] == "upwards")
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # CALCULATE WT OVERSOLD
        dataframe.loc[(dataframe["trend"] == "downwards"), "wtOverbought"] = (
            dataframe["wt2"] >= self.sell_downtrend_oblevel.value
        )
        dataframe.loc[(dataframe["trend"] == "sideways"), "wtOverbought"] = (
            dataframe["wt2"] >= self.sell_sideways_oblevel.value
        )
        dataframe.loc[(dataframe["trend"] == "upwards"), "wtOverbought"] = (
            dataframe["wt2"] >= self.sell_uptrend_oblevel.value
        )
        ...
        ] = self.sell_downtrend_wavetrend_weight.value
        dataframe.loc[
            (dataframe["trend"] == "sideways")
            & (dataframe["wtCrossDown"] & dataframe["wtOverbought"]),
            "sell_wavetrend_weight",
        ] = self.sell_sideways_wavetrend_weight.value
        dataframe.loc[
            (dataframe["trend"] == "upwards")
            & (dataframe["wtCrossDown"] & dataframe["wtOverbought"]),
            "sell_wavetrend_weight",
        ] = self.sell_uptrend_wavetrend_weight.value

        dataframe["total_sell_signal_strength"] += dataframe["sell_wavetrend_weight"]

        # rsi div and rsi va̱ur buy_rsi_div buy_rsi_div_weight
        dataframe.loc[
            (dataframe["trend"] == "downwards") & (dataframe["bearish_div"]),
            "sell_rsi_divergence_weight",
        ] = self.sell_downtrend_rsi_divergence_weight.value
        dataframe.loc[
            (dataframe["trend"] == "sideways") & (dataframe["bearish_div"]),
            "sell_rsi_divergence_weight",
        ] = self.sell_sideways_rsi_divergence_weight.value
        dataframe.loc[
            (dataframe["trend"] == "upwards") & (dataframe["bearish_div"]),
            "sell_rsi_divergence_weight",
        ] = self.sell_uptrend_rsi_divergence_weight.value
        ...
        dataframe.loc[
            (
                ...
                & (
                    ...
                    >= self.sell_downtrend_total_signal_needed.value
                )
            )
            | (
                (dataframe["trend"] == "sideways")
                & (
                    dataframe["total_sell_signal_strength"]
                    >= self.sell_sideways_total_signal_needed.value
                )
            )
            | (
                (dataframe["trend"] == "upwards")
