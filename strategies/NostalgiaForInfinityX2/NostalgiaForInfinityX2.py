...
class NostalgiaForInfinityX2(IStrategy):
    ...
    def sell_long_bull(
        ...
        sell, signal_name = self.sell_long_bull_signals(
            current_profit,
            max_profit,
            max_loss,
            last_candle,
        ...
        # Main sell signals
        sell, signal_name = self.sell_long_bull_main(
            current_profit,
            max_profit,
            max_loss,
            last_candle,
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if all(c in ["1"] for c in buy_tags):
            sell, signal_name = self.sell_long_bull(
                current_profit,
                max_profit,
                max_loss,
                last_candle,
    ...
    def informative_pairs(self):
        ...
        informative_pairs.extend(
            [(btc_info_pair, btc_info_timeframe) for btc_info_timeframe in self.btc_info_timeframes]
        )
    ...
    # BTC 1D Indicators
    # ---------------------------------------------------------------------------------------------
    def btc_info_1d_indicators(
        self, btc_info_pair, btc_info_timeframe, metadata: dict
    ) -> DataFrame:
        tik = time.perf_counter()
    ...
    def btc_info_switcher(self, btc_info_pair, btc_info_timeframe, metadata: dict) -> DataFrame:
        if btc_info_timeframe == "1d":
            ...
            raise RuntimeError(
                f"{btc_info_timeframe} not supported as informative timeframe for BTC pair."
            )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for btc_info_timeframe in self.btc_info_timeframes:
            ...
            dataframe = merge_informative_pair(
                dataframe, btc_informative, self.timeframe, btc_info_timeframe, ffill=True
            )
            # Customize what we drop - in case we need to maintain some BTC informative ohlcv data
            # Default drop all
            drop_columns = {
                "1d": [
                    f"btc_{s}_{btc_info_timeframe}"
        ...
        for info_timeframe in self.info_timeframes:
            ...
            dataframe = merge_informative_pair(
                dataframe, info_indicators, self.timeframe, info_timeframe, ffill=True
            )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if all(c in ["11"] for c in buy_tags):
            sell, signal_name = self.sell_long_bear(
                current_profit,
                max_profit,
                max_loss,
                last_candle,
