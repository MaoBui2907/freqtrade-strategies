...
sell_params = {"base_nb_candles_sell": 19, "high_offset": 1.008}
...
def Kalman(dataframe: DataFrame):
    ...
    df["value1"] = (
        df["value1"].rolling(2, min_periods=1).apply(lambda x: x.iloc[0] + 0.8 * x.iloc[-1])
    )
    df["value2"] = 0.1 * (tr)
    df["value2"] = (
        df["value2"].rolling(2, min_periods=1).apply(lambda x: x.iloc[0] + 0.8 * x.iloc[-1])
    )
