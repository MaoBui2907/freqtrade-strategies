...
class NostalgiaForInfinityNext_ChangeToTower_V5_3(IStrategy):
    ...
    def load_hold_trades_config(self):
        ...
        hold_trades_config_file = (
            NostalgiaForInfinityNext_ChangeToTower_V5_3.get_hold_trades_config_file()
        )
    ...
    def sell_quick_mode(
        ...
        if (
            ...
            and (last_candle["cci"] > 200)
            and (last_candle["hrsi"] > 80)
            and (last_candle["moderi_32"] is True)
            and (current_profit > 0)
        ):
            return True, "signal_profit_zlema_up"

        if (
            (last_candle["zlema_4_std"] > last_candle["zlema_1_std"])
            and (previous_candle_1["zlema_4_std"] < previous_candle_1["zlema_1_std"])
            and (last_candle["cci"] > -100)
            and (last_candle["hrsi"] > 80)
            and (last_candle["moderi_32"] is False)
            and (current_profit > 0)
        ):
