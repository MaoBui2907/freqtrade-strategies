...
class Obelisk_Ichimoku_ZEMA_v1(IStrategy):
    ...
    def slow_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["trend_pulse"] = (
            (dataframe["ichimoku_ok"] > 0) & (dataframe["ssl_ok"] > 0)
        ).astype("int") * 2

        dataframe["bear_trend_pulse"] = (
            (dataframe["ichimoku_bear"] > 0) & (dataframe["ssl_bear"] > 0)
        ).astype("int") * 2

        dataframe["trend_over"] = (
            (dataframe["ssl_ok"] == 0) | (dataframe["close"] < dataframe["cloud_top"])
        ).astype("int") * 1

        dataframe["bear_trend_over"] = (
            (dataframe["ssl_bear"] == 0) | (dataframe["close"] > dataframe["cloud_bottom"])
        ).astype("int") * 1
    ...
    def fast_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if self.config["runmode"].value == "hyperopt":
            ...
            dataframe["zema_buy"] = (
                ftt.zema(dataframe, period=self.zema_len_buy.value) * self.low_offset.value
            )
            dataframe["zema_sell"] = (
                ftt.zema(dataframe, period=self.zema_len_sell.value) * self.high_offset.value
            )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (dataframe["close"] > (dataframe[zema] * self.high_offset.value)), "exit_long"
        ] = 1
