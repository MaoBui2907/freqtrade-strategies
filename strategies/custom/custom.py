...
class custom(IStrategy):
    ...
    # SMAOffsetProtectOptV1
    low_offset2 = DecimalParameter(
        0.9, 0.99, default=buy_params["low_offset2"], space="entry", optimize=True
    )
    base_nb_candles_buy2 = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy2"], space="entry", optimize=True
    )
    base_nb_candles_sell2 = IntParameter(
        5, 80, default=sell_params["base_nb_candles_sell"], space="exit", optimize=True
    )
    high_offset2 = DecimalParameter(
        0.95, 1.1, default=sell_params["high_offset"], space="exit", optimize=True
    )
    ...
    ewo_high = DecimalParameter(
        2.00, 12.0, default=buy_params["ewo_high"], space="entry", optimize=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["prevMin"] = np.where(
            dataframe["min"] > dataframe["min"].shift(), dataframe["min"].shift(), dataframe["min"]
        )
        ...
        dataframe["prevMax"] = np.where(
            dataframe["max"] < dataframe["max"].shift(), dataframe["max"].shift(), dataframe["max"]
        )
        ...
        dataframe.loc[
            ((dataframe["priceLL"] == 1) & (dataframe["oscHL"] == 1) & (dataframe["plFound"] == 1)),
            "bullCond",
        ] = 1
        ...
        dataframe.loc[
            ((dataframe["priceHL"] == 1) & (dataframe["oscLL"] == 1) & (dataframe["plFound"] == 1)),
            "hiddenBullCond",
        ] = 1
        ...
        dataframe.loc[
            ((dataframe["priceHH"] == 1) & (dataframe["oscLH"] == 1) & (dataframe["phFound"] == 1)),
            "bearCond",
        ] = 1
        ...
        dataframe.loc[
            ((dataframe["priceLH"] == 1) & (dataframe["oscHH"] == 1) & (dataframe["phFound"] == 1)),
            "hiddenBearCond",
        ] = 1
        ...
        conditions.append(
            (
                ...
                & (
                    dataframe["close"]
                    < (
                        dataframe[f"ma_buy2_{self.base_nb_candles_buy2.value}"]
                        * self.low_offset2.value
                    )
                )
...
def valuewhen(dataframe, condition, source, occurrence):
    ...
    copy.loc[
        ((rangeLower <= copy["barrsince"]) & (copy["barrsince"] <= rangeUpper)), "in_range"
    ] = 1
