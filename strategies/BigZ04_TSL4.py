...
class BigZ04_TSL4(IStrategy):
    ...
    buy_bb20_close_bblowerband_safe_1 = DecimalParameter(
        0.950, 1.050, default=0.989, decimals=3, space="entry", optimize=False, load=True
    )
    buy_bb20_close_bblowerband_safe_2 = DecimalParameter(
        0.700, 1.100, default=0.982, decimals=2, space="entry", optimize=False, load=True
    )
    ...
    buy_rsi_1h_0 = DecimalParameter(
        55.0, 85.0, default=71.0, space="entry", decimals=1, optimize=False, load=True
    )
    buy_rsi_1h_1a = DecimalParameter(
        65.0, 78.0, default=69.0, space="entry", decimals=1, optimize=False, load=True
    )
    ...
    buy_rsi_0 = DecimalParameter(
        10.0, 40.0, default=30.0, space="entry", decimals=1, optimize=False, load=True
    )
    ...
    buy_dip_0 = DecimalParameter(
        1.015, 1.040, default=1.024, space="entry", decimals=3, optimize=False, load=True
    )
    ...
    narrow_stop = DecimalParameter(
        1.005, 1.030, default=1.020, space="exit", decimals=3, optimize=False, load=True
    )
    wide_stop = DecimalParameter(
        1.010, 1.045, default=1.035, space="exit", decimals=3, optimize=False, load=True
    )

    # hyperopt parameters for SMAOffsetProtectOptV1 sell signal
    base_nb_candles_sell = IntParameter(5, 80, default=49, space="exit", optimize=False, load=True)
    high_offset = DecimalParameter(
        0.99, 1.1, default=1.006, space="exit", optimize=False, load=True
    )

    # trailing stoploss hyperopt parameters
    # hard stoploss profit
    pHSL = DecimalParameter(
        -0.200, -0.040, default=-0.08, decimals=3, space="exit", optimize=False, load=True
    )
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(
        0.008, 0.020, default=0.016, decimals=3, space="exit", optimize=False, load=True
    )
    pSL_1 = DecimalParameter(
        0.008, 0.020, default=0.011, decimals=3, space="exit", optimize=False, load=True
    )

    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(
        0.040, 0.100, default=0.080, decimals=3, space="exit", optimize=False, load=True
    )
    pSL_2 = DecimalParameter(
        0.020, 0.070, default=0.040, decimals=3, space="exit", optimize=False, load=True
    )
