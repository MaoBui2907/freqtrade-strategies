...
class ClucHAnix_hhll(IStrategy):
    ...
    bbdelta_close = RealParameter(
        0.0005, 0.02, default=0.01965, space="entry", optimize=is_optimize_clucHA
    )
    closedelta_close = RealParameter(
        0.0005, 0.02, default=0.00556, space="entry", optimize=is_optimize_clucHA
    )
    bbdelta_tail = RealParameter(
        0.7, 1.0, default=0.95089, space="entry", optimize=is_optimize_clucHA
    )
    close_bblower = RealParameter(
        0.0005, 0.02, default=0.00799, space="entry", optimize=is_optimize_clucHA
    )
    ...
    max_slip = DecimalParameter(
        0.33, 0.80, default=0.33, decimals=3, optimize=is_optimize_slip, space="entry", load=True
    )
    ...
    sell_bbmiddle_close = RealParameter(
        0.97, 1.1, default=1.07634, space="exit", optimize=is_optimize_sell
    )
    high_offset = DecimalParameter(
        0.90, 1.2, default=sell_params["high_offset"], space="exit", optimize=is_optimize_sell
    )
    high_offset_2 = DecimalParameter(
        0.90, 1.5, default=sell_params["high_offset_2"], space="exit", optimize=is_optimize_sell
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["hh_48_diff"] = (
            (dataframe["hh_48"] - dataframe["close"]) / dataframe["hh_48"] * 100
        )

        # ll48
        dataframe["ll_48"] = ta.MIN(dataframe["low"], 48)
        dataframe["ll_48_diff"] = (
            (dataframe["close"] - dataframe["ll_48"]) / dataframe["ll_48"] * 100
        )
