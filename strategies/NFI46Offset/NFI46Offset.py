...
class NFI46Offset(IStrategy):
    ...
    # Multi Offset
    base_nb_candles_buy = IntParameter(5, 80, default=20, load=True, space="entry", optimize=True)
    base_nb_candles_sell = IntParameter(5, 80, default=20, load=True, space="exit", optimize=True)
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in self.ma_types:
            dataframe[f"{i}_offset_buy"] = (
                self.ma_map[f"{i}"]["calculate"](dataframe, self.base_nb_candles_buy.value)
                * self.ma_map[f"{i}"]["low_offset"]
            )
            dataframe[f"{i}_offset_sell"] = (
                self.ma_map[f"{i}"]["calculate"](dataframe, self.base_nb_candles_sell.value)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in self.ma_types:
            conditions.append(
                ((dataframe["close"] > dataframe[f"{i}_offset_sell"]) & (dataframe["volume"] > 0))
            )
