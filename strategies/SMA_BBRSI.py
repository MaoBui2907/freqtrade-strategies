...
class SMA_BBRSI(IStrategy):
    ...
    ewo_high_bb = DecimalParameter(
        0, 7.0, default=buy_params["ewo_high_bb"], space="entry", optimize=True
    )
    for_sigma = DecimalParameter(
        0, 10.0, default=buy_params["for_sigma"], space="entry", optimize=True
    )
    for_sigma_sell = DecimalParameter(
        0, 10.0, default=sell_params["for_sigma_sell"], space="exit", optimize=True
    )
    rsi_high = IntParameter(60, 100, default=sell_params["rsi_high"], space="exit", optimize=True)
    for_ma_length = IntParameter(
        5, 80, default=buy_params["for_ma_length"], space="entry", optimize=True
    )
    for_ma_length_sell = IntParameter(
        5, 80, default=sell_params["for_ma_length_sell"], space="exit", optimize=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["cci_slow"] = pta.cci(
            high=dataframe["high"], low=dataframe["low"], close=dataframe["close"], length=240
        )
        dataframe["cci_fast"] = pta.cci(
            high=dataframe["high"], low=dataframe["low"], close=dataframe["close"], length=20
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dont_buy_conditions.append((dataframe["pump_strength"] > self.antipump_threshold.value))
            ...
            (
                (
                    ...
                    < (
                        ...
                        - (dataframe[f"dev_{self.for_ma_length.value}"] * self.for_sigma.value)
                    )
                )
                & (
                    ((dataframe["EWO"] > self.ewo_high_bb.value) & (dataframe["EWO"] < 10))
