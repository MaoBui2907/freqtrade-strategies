...
class TrixV23Strategy(IStrategy):
    ...
    buy_trix_signal_type = CategoricalParameter(
        ["trailing", "trigger"], default="trigger", space="buy", optimize=False, load=True
    )
    buy_trix_src = CategoricalParameter(
        ["open", "high", "low", "close"], default="close", space="buy", optimize=False, load=True
    )
    ...
    buy_trix_signal_timeperiod = IntParameter(
        5, 25, default=21, space="buy", optimize=False, load=True
    )
    ...
    buy_stoch_rsi = DecimalParameter(
        0.6, 0.99, decimals=3, default=0.987, space="buy", optimize=False, load=True
    )
    buy_stoch_rsi_timeperiod = IntParameter(
        5, 25, default=14, space="buy", optimize=False, load=True
    )
    ...
    buy_ema_multiplier = DecimalParameter(
        0.8, 1.2, decimals=2, default=1.00, space="buy", optimize=False, load=True
    )
    ...
    buy_btc_ema_timeperiod = IntParameter(
        150, 250, default=200, space="buy", optimize=True, load=True
    )
    buy_btc_ema_multiplier = DecimalParameter(
        0.8, 1.0, decimals=3, default=0.97, space="buy", optimize=True, load=True
    )

    # sell

    sell_trix_signal_type = CategoricalParameter(
        ["trailing", "trigger"], default="trailing", space="sell", optimize=False, load=True
    )
    sell_trix_src = CategoricalParameter(
        ["open", "high", "low", "close"], default="close", space="sell", optimize=False, load=True
    )
    ...
    sell_trix_signal_timeperiod = IntParameter(
        5, 25, default=21, space="sell", optimize=False, load=True
    )
    ...
    sell_stoch_rsi = DecimalParameter(
        0.01, 0.4, decimals=3, default=0.048, space="sell", optimize=False, load=True
    )
    sell_stoch_rsi_timeperiod = IntParameter(
        5, 25, default=14, space="sell", optimize=False, load=True
    )
    ...
    sell_atr_multiplier = DecimalParameter(
        0.7, 9.0, decimals=3, default=4.0, space="sell", optimize=False, load=True
    )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        stoploss_price = (
            last_candle["low"]
            - last_candle[f"atr_{self.sell_atr_timeperiod.value}"] * self.sell_atr_multiplier.value
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for val in self.buy_trix_timeperiod.range:
            dataframe[f"trix_b_{val}"] = ta.EMA(
                ta.EMA(ta.EMA(dataframe[self.buy_trix_src.value], timeperiod=val), timeperiod=val),
                timeperiod=val,
            )
        ...
        for val in self.sell_trix_timeperiod.range:
            dataframe[f"trix_s_{val}"] = ta.EMA(
                ta.EMA(ta.EMA(dataframe[self.sell_trix_src.value], timeperiod=val), timeperiod=val),
                timeperiod=val,
            )
        ...
        for val in self.buy_stoch_rsi_timeperiod.range:
            dataframe["b_stoch_rsi"] = (
                dataframe["b_rsi"] - dataframe["b_rsi"].rolling(val).min()
            ) / (dataframe["b_rsi"].rolling(val).max() - dataframe["b_rsi"].rolling(val).min())
        ...
        for val in self.sell_stoch_rsi_timeperiod.range:
            dataframe["s_stoch_rsi"] = (
                dataframe["s_rsi"] - dataframe["s_rsi"].rolling(val).min()
            ) / (dataframe["s_rsi"].rolling(val).max() - dataframe["s_rsi"].rolling(val).min())
        ...
        dataframe["ema_b_signal"] = (
            dataframe[f"ema_b_{self.buy_ema_timeperiod.value}"] * self.buy_ema_multiplier.value
        )

        # ATR
        for val in self.sell_atr_timeperiod.range:
            dataframe[f"atr_{val}"] = ta.ATR(
                dataframe["high"], dataframe["low"], dataframe["close"], timeperiod=val
            )
        dataframe["stoploss_price"] = (
            dataframe["low"]
            - dataframe[f"atr_{self.sell_atr_timeperiod.value}"] * self.sell_atr_multiplier.value
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_btc_ema_enabled.value:
            conditions.append(
                dataframe[f"btc_{stake}_close_1h"]
                > (
                    dataframe[f"btc_{stake}_ema_{self.buy_btc_ema_timeperiod.value}_1h"]
                    * self.buy_btc_ema_multiplier.value
                )
            )
