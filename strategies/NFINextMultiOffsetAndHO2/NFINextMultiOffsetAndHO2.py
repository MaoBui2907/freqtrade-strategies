...
class NFINextMultiOffsetAndHO2(IStrategy):
    ...
    buy_mfi_1 = DecimalParameter(
        20.0, 40.0, default=44.0, space="entry", decimals=1, optimize=False, load=True
    )
    ...
    buy_rsi_8 = DecimalParameter(
        20.0, 50.0, default=30.0, space="entry", decimals=1, optimize=False, load=True
    )
    buy_rsi_1h_8 = DecimalParameter(
        40.0, 66.0, default=54.0, space="entry", decimals=1, optimize=False, load=True
    )
    ...
    buy_27_wr_max = DecimalParameter(
        95, 99, default=95.4, space="entry", decimals=1, optimize=False, load=True
    )
    ...
    def sell_quick_mode(
        self, current_profit: float, max_profit: float, last_candle, buy_signal_candle
    ) -> tuple:
        if (
        ...
        **kwargs,
        ...
        if not buy_signal.empty:
            sell, signal_name = self.sell_quick_mode(
                current_profit, max_profit, last_candle, buy_signal_candle
            )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.config["runmode"].value in ("live", "dry_run"):
            ...
            not_empty_volume = (
                dataframe["volume"]
                .rolling(window=self.startup_candle_count, min_periods=self.startup_candle_count)
                .count()
                .notna()
            )
