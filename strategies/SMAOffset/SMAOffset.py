...
class SMAOffset(IStrategy):
    ...
    base_nb_candles_buy = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy"], space="entry"
    )
    base_nb_candles_sell = IntParameter(
        5, 80, default=sell_params["base_nb_candles_sell"], space="exit"
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            ((dataframe["close"] < dataframe["ma_offset_buy"]) & (dataframe["volume"] > 0)),
            "enter_long",
        ] = 1
        return dataframe
