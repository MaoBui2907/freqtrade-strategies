...
class NASOSRv6_private_Reinuvader_20211121(IStrategy):
    ...
    base_nb_candles_buy = IntParameter(
        2, 26, default=buy_params["base_nb_candles_buy"], space="entry", optimize=False
    )
    base_nb_candles_sell = IntParameter(
        2, 20, default=sell_params["base_nb_candles_sell"], space="exit", optimize=False
    )
    low_offset = DecimalParameter(
        0.85, 0.99, default=buy_params["low_offset"], space="entry", optimize=False
    )
    ...
    high_offset_2 = DecimalParameter(
        0.99, 1.5, default=sell_params["high_offset_2"], space="exit", optimize=False
    )
    ...
    lookback_candles = IntParameter(
        1, 36, default=buy_params["lookback_candles"], space="entry", optimize=False
    )
    profit_threshold = DecimalParameter(
        1.0, 1.08, default=buy_params["profit_threshold"], space="entry", optimize=False
    )
    ewo_low = DecimalParameter(
        -20.0, -8.0, default=buy_params["ewo_low"], space="entry", optimize=False
    )
    ...
    pHSL = DecimalParameter(
        -0.15, -0.08, default=sell_params["pHSL"], decimals=3, space="exit", optimize=True
    )
    ProfitMargin1 = DecimalParameter(
        0.009, 0.019, default=sell_params["ProfitMargin1"], decimals=3, space="exit", optimize=True
    )
    ProfitLoss1 = DecimalParameter(
        0.005, 0.012, default=sell_params["ProfitLoss1"], decimals=3, space="exit", optimize=True
    )
    ProfitMargin2 = DecimalParameter(
        0.033, 0.099, default=sell_params["ProfitMargin2"], decimals=3, space="exit", optimize=True
    )
    ProfitLoss2 = DecimalParameter(
        0.010, 0.025, default=sell_params["ProfitLoss2"], decimals=3, space="exit", optimize=True
    )
    ...
    clucha_rocr_1h = DecimalParameter(
        0.1, 1.0, default=buy_params["clucha_rocr_1h"], decimals=5, space="entry", optimize=False
    )

    # Zema
    base_nb_candles_buy_zema = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy_zema"], space="entry", optimize=False
    )
    low_offset_zema = DecimalParameter(
        0.9, 0.99, default=buy_params["low_offset_zema"], space="entry", optimize=False
    )
    base_nb_candles_buy_zema2 = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy_zema2"], space="entry", optimize=False
    )
    low_offset_zema2 = DecimalParameter(
        0.9, 0.99, default=buy_params["low_offset_zema2"], space="entry", optimize=False
    )
    high_offset_sell_ema = DecimalParameter(
        0.99, 1.1, default=buy_params["high_offset_sell_ema"], space="entry", optimize=False
    )
    base_nb_candles_buy_ema = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy_ema"], space="entry", optimize=False
    )
    low_offset_ema = DecimalParameter(
        0.9, 1.1, default=buy_params["low_offset_ema"], space="entry", optimize=False
    )
    base_nb_candles_buy_ema2 = IntParameter(
        5, 80, default=buy_params["base_nb_candles_buy_ema2"], space="entry", optimize=False
    )
    low_offset_ema2 = DecimalParameter(
        0.9, 1.1, default=buy_params["low_offset_ema2"], space="entry", optimize=False
    )
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["long_term_price_warning"] = np.where(
            (
                ...
                # 25% drop in 24h
                (
                    dataframe["close"].rolling(288).max() * 0.75
                    > dataframe["close"].rolling(12).mean()
                )
                |
                # 30% drop in 36h
                (
                    dataframe["close"].rolling(432).max() * 0.7
                    > dataframe["close"].rolling(24).mean()
                )
                |
                # over 35% increase in the past 24h and 36h
                (
                    (
                        df24h["close"].rolling(24).mean() * 1.42
                        < dataframe["close"].rolling(18).mean()
                    )
                    & (
                        df36h["close"].rolling(24).mean() * 1.42
                        < dataframe["close"].rolling(18).mean()
                    )
                )
            ),
        ...
        for period in self.timeperiods:
            ...
            dataframe[f"linangle_{period}"] = normalize(
                ta.LINEARREG_ANGLE(dataframe, timeperiod=period), -35, 35
            )

        # MACD
        dataframe["close_min"] = (
            dataframe["close"].rolling(window=960 // self.timeframe_minutes).min()
        )
        dataframe["close_max"] = (
            dataframe["close"].rolling(window=960 // self.timeframe_minutes).max()
        )
        dataframe["relative_price"] = normalize(
            dataframe["close"], dataframe["close_min"], dataframe["close_max"]
        )

        # macd = ta.MACD(dataframe)
        dataframe["macd"], dataframe["macdsignal"], dataframe["macdhist"] = ta.MACD(
            dataframe["close"], fastperiod=21, slowperiod=45, signalperiod=16
        )
        ...
        dataframe["macd_norm"] = np.where(
            dataframe["macdmin"] == dataframe["macdmax"],
            0,
            (
                2.0
                * (dataframe["macd"] - dataframe["macdmin"])
                / (dataframe["macdmax"] - dataframe["macdmin"])
                - 1.0
            ),
        )
            ...
            (
                ...
                & (
                    ...
                    | (
                        ...
                        & (
                            dataframe["ha_close"]
                            < self.clucha_close_bblower.value * dataframe["lower"]
                        )
                    )
                )
            ),
