...
class Obelisk_3EMA_StochRSI_ATR(IStrategy):
    ...
    def do_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[qtpylib.crossed_above(dataframe["go_long"], 0), "take_profit"] = (
            dataframe["close"] + dataframe["atr"] * 2
        )
        dataframe["take_profit"].fillna(method="ffill", inplace=True)

        dataframe.loc[qtpylib.crossed_above(dataframe["go_long"], 0), "stop_loss"] = (
            dataframe["close"] - dataframe["atr"] * 3
        )
        dataframe["stop_loss"].fillna(method="ffill", inplace=True)

        dataframe.loc[qtpylib.crossed_above(dataframe["go_long"], 0), "stop_pct"] = (
            dataframe["atr"] * 3
        ) / dataframe["close"]
        ...
        self.custom_info[metadata["pair"]] = (
            dataframe[["date", "stop_pct", "take_profit"]].copy().set_index("date")
        )
        ...
        **kwargs,
        ...
        if custom_info_pair is not None:
            ...
            open_date = (
                ...
                else trade.open_date.replace(tzinfo=custom_info_pair.index.tz)
            )
            ...
            if stop_pct is not None:
                ...
                logger.debug(
                    "open={}, current={}, profit={}, stop_pct={}, stop={}".format(
