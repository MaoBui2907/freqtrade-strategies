...
class TheRealPullbackV2(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["pvo"] = (
            (dataframe["fastEMA"] - dataframe["slowEMA"]) / dataframe["slowEMA"]
        ) * 100
        ...
        dataframe["buy_signal"] = ((dataframe["is_dip"] > 0) & (dataframe["is_break"] > 0)).astype(
            "int"
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe["buy_signal"] > 0), "enter_long"] = 1
