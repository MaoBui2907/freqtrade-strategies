...
def pump_warning2(dataframe, params):
    ...
    df["pct_change_int"] = (df["pct_change"] > pct_change_max).astype("int") | (
        df["pct_change"] < pct_change_min
    ).astype("int")
    df["pct_change_short"] = df["close"].pct_change(periods=pct_change_short_timeframe)
    df["pct_change_int_short"] = (df["pct_change_short"] > pct_change_short_max).astype("int") | (
        df["pct_change_short"] < pct_change_short_min
    ).astype("int")
    df["ispumping"] = (df["pct_change_int"].rolling(ispumping_rolling).sum() >= ispumping).astype(
        "int"
    )
    df["islongpumping"] = (
        df["pct_change_int"].rolling(islongpumping_rolling).sum() >= islongpumping
    ).astype("int")
    df["isshortpumping"] = (
        df["pct_change_int_short"].rolling(isshortpumping_rolling).sum() >= isshortpumping
    ).astype("int")
    df["recentispumping"] = (
        (df["ispumping"].rolling(recentispumping_rolling).max() > 0)
        | (df["islongpumping"].rolling(recentispumping_rolling).max() > 0)
        | (df["isshortpumping"].rolling(recentispumping_rolling).max() > 0)
    )
...
def dump_warning(dataframe, buy_threshold):
    ...
    # 5m dump protection
    df_past_source = (
        df_past["open"] + df_past["close"] + df_past["high"] + df_past["low"]
    ) / 4  # Get BTC price
    ...
    df_past_source_1d = (
        df_past_1d["open"] + df_past_1d["close"] + df_past_1d["high"] + df_past_1d["low"]
    ) / 4
...
class BB_RPB_TSL_SMA_Tranz(IStrategy):
    ...
    lambo2_ema_14_factor = DecimalParameter(
        0.9, 0.99, default=buy_params["lambo2_ema_14_factor"], space="entry", optimize=True
    )
    lambo2_rsi_4_limit = IntParameter(
        2, 50, default=buy_params["lambo2_rsi_4_limit"], space="entry", optimize=True
    )
    lambo2_rsi_14_limit = IntParameter(
        2, 50, default=buy_params["lambo2_rsi_14_limit"], space="entry", optimize=True
    )
    ...
    base_nb_candles_buy_ema = IntParameter(
        5, 80, default=20, space="entry", optimize=optimize_buy_ema
    )
    low_offset_ema = DecimalParameter(
        0.9, 1.1, default=0.958, space="entry", optimize=optimize_buy_ema
    )
    ...
    profit_threshold = DecimalParameter(
        0.99, 1.05, default=buy_params["profit_threshold"], space="entry", optimize=True
    )
    ...
    ewo_low2 = DecimalParameter(
        -20.0, -8.0, default=buy_params["ewo_low2"], space="entry", optimize=True
    )
    ewo_high2 = DecimalParameter(
        2.0, 12.0, default=buy_params["ewo_high2"], space="entry", optimize=True
    )
    ...
    buy_ema_open_mult_15 = DecimalParameter(
        0.01, 0.03, default=0.024, optimize=is_optimize_ctt15_protection
    )
    buy_ma_offset_15 = DecimalParameter(
        0.93, 0.99, default=0.958, optimize=is_optimize_ctt15_protection
    )
    buy_rsi_15 = DecimalParameter(20.0, 36.0, default=28.0, optimize=is_optimize_ctt15_protection)
    buy_ema_rel_15 = DecimalParameter(
        0.97, 0.999, default=0.974, optimize=is_optimize_ctt15_protection
    )

    is_optimize_ctt25_protection = False
    buy_25_ma_offset = DecimalParameter(
        0.90, 0.99, default=0.922, optimize=is_optimize_ctt25_protection
    )
    ...
    max_slip = DecimalParameter(
        0.33, 1.00, default=0.33, decimals=3, optimize=is_optimize_slip, space="buy", load=True
    )
    ...
    buy_12_protection__sma200_rising_val = CategoricalParameter(
        ["20", "30", "36", "44", "50"], default="50", space="buy", optimize=False, load=True
    )
    buy_12_protection__sma200_1h_rising = CategoricalParameter(
        [True, False], default=True, space="buy", optimize=False, load=True
    )
    buy_12_protection__sma200_1h_rising_val = CategoricalParameter(
        ["20", "30", "36", "44", "50"], default="24", space="buy", optimize=False, load=True
    )
    ...
    pump_protection_01_pct_change_timeframe = IntParameter(
        5, 10, default=8, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_pct_change_max = DecimalParameter(
        0.05, 0.35, default=0.15, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_pct_change_min = DecimalParameter(
        -0.35, -0.05, default=-0.15, space="buy", optimize=optimize_buy_pump_protection_01
    )

    pump_protection_01_pct_change_short_timeframe = IntParameter(
        5, 10, default=8, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_pct_change_short_max = DecimalParameter(
        0.05, 0.35, default=0.08, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_pct_change_short_min = DecimalParameter(
        -0.35, -0.05, default=-0.08, space="buy", optimize=optimize_buy_pump_protection_01
    )

    pump_protection_01_ispumping = DecimalParameter(
        0.05, 0.35, default=0.2, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_islongpumping = DecimalParameter(
        0.05, 0.35, default=0.24, space="buy", optimize=optimize_buy_pump_protection_01
    )
    pump_protection_01_isshortpumping = DecimalParameter(
        0.05, 0.35, default=0.12, space="buy", optimize=optimize_buy_pump_protection_01
    )
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h["bb20_width"] = (
            informative_1h["bb20_2_upp"] - informative_1h["bb20_2_low"]
        ) / informative_1h["bb20_2_mid"]
    ...
    def informative_15m_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_15m["sma_200_dec_20"] = informative_15m["sma_200"] < informative_15m[
            "sma_200"
        ].shift(20)

        # BB
        bollinger = qtpylib.bollinger_bands(
            qtpylib.typical_price(informative_15m), window=20, stds=2
        )
        ...
        informative_15m["closedelta"] = (
            informative_15m["close"] - informative_15m["close"].shift()
        ).abs()
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["recentispumping"] = pump_warning2(
            dataframe,
            {
        ...
        # MAMA
        dataframe["mama"], dataframe["fama"] = ta.MAMA(
            dataframe["close"], fastlimit=0.5, slowlimit=0.05
        )
        ...
        dataframe["zema_offset_buy"] = (
            zema(dataframe, int(self.base_nb_candles_buy_zema.value)) * self.low_offset_zema.value
        )
        dataframe["zema_offset_buy2"] = (
            zema(dataframe, int(self.base_nb_candles_buy_zema2.value)) * self.low_offset_zema2.value
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        buy_profit = dataframe["close_1h"].rolling(24).max() > (dataframe["close"] * 1.03)
        ...
        is_can_buy_smooth_ha = (dataframe["close"] < dataframe["Smooth_HA_L"]) & (
        ...
        is_standart_prot = (is_btc_safe | is_pair_safe & nfi7_sma_protection) | (
            is_MMA_prot & nfi7_sma_protection
        )
        ...
        is_nfix_53 = (
            ...
            & (
                (dataframe["ema_26_15m"].shift(3) - dataframe["ema_12_15m"].shift(3))
                > (dataframe["open_15m"] / 100)
            )
        ...
        is_nfix_47 = (
            ...
            & (
                (
                    (dataframe["open_15m"] < dataframe["ema_20_1h"])
                    & (dataframe["low_15m"] < dataframe["ema_20_1h"])
                )
                | (
                    (dataframe["open_15m"] > dataframe["ema_20_1h"])
                    & (dataframe["low_15m"] > dataframe["ema_20_1h"])
...
class UziChanTB2(BB_RPB_TSL_SMA_Tranz):
    ...
    def trailing_sell_offset(self, dataframe, pair: str, current_price: float):
        ...
        if trailing_duration.total_seconds() > self.trailing_expire_seconds:
            ...
            self.trailing_sell_uptrend_enabled
            and (trailing_duration.total_seconds() < self.trailing_expire_seconds_uptrend)
                    ...
                    if trailing_buy["allow_trailing"]:
                        if not trailing_buy["trailing_buy_order_started"] and (
                            ...
                            if trailing_buy_offset == "force_enter":
                                ...
                                self.custom_info_trail_buy[pair]["trailing_buy"]["offset"] = (
                                    trailing_buy_offset
                                )
                                self.trailing_buy_info(pair, current_price)
                                logger.info(
                    ...
                    if trailing_sell["allow_sell_trailing"]:
                        if not trailing_sell["trailing_sell_order_started"] and (
                            ...
                            if trailing_sell_offset == "force_exit":
                                ...
                                self.custom_info_trail_sell[pair]["trailing_sell"]["offset"] = (
                                    trailing_sell_offset
                                )
                                self.trailing_sell_info(pair, current_price)
                                logger.info(
        ...
        if self.trailing_sell_order_enabled and self.config["runmode"].value in ("live", "dry_run"):
            ...
            if last_candle["exit_long"] != 0:
                if not trailing_sell["trailing_sell_order_started"]:
                    ...
                    if open_trades:
                        logger.info(
                            f"Set 'allow_SELL_trailing' to True for {metadata['pair']} to start *SELL* trailing"
                        )
                        # self.custom_info_trail_buy[metadata['pair']]['trailing_buy']['allow_trailing'] = True
                        trailing_sell["allow_sell_trailing"] = True
                        initial_sell_tag = (
                            last_candle["sell_tag"] if "sell_tag" in last_candle else "sell signal"
                        )
