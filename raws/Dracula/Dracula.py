...
class SupResFinder:
    def isSupport(self, df, i):
        support = (
            df["bb_bbl_i"][i] == 1
            and (df["bb_bbl_i"][i + 1] == 0 or df["close"][i + 1] > df["open"][i + 1])
    ...
    def isResistance(self, df, i):
        resistance = (
            df["bb_bbh_i"][i] == 1
            and (df["bb_bbh_i"][i + 1] == 0 or df["close"][i + 1] < df["open"][i + 1])
            and df["close"][i] > df["open"][i]
        )
...
class Dracula(IStrategy):
    ...
    buy_bbt = DecimalParameter(0, 100, decimals=4, default=0.023, space="entry")
    # Buy hypers
    timeframe = "1m"
    # Protection
    fast_ewo = 50
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[reduce(lambda x, y: x & y, item_buy_logic), ["entry", "buy_tag"]] = (
            1,
            "buy_1",
        )
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if (
            ...
            (last_candle["close"] < last_candle["open"])
            and (last_candle["close"] <= (last_candle["bb_bbl"] * 1.002))
            and current_profit >= 0
        ):
            return "take_profit_" + trade.buy_tag
        elif "sma" in trade.buy_tag and current_profit >= 0.01:
            return "sma"
        elif "sma" in trade.buy_tag and (
            last_candle["close"] > (last_candle["ema_49"] * self.high_offset.value)
        ):
