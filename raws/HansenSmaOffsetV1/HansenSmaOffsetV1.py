...
class HansenSmaOffsetV1(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["smau1"] = ta.SMA(dataframe["close"], timeperiod=20) + 0.05 * ta.SMA(
            dataframe["close"], timeperiod=20
        )
        dataframe["smad1"] = ta.SMA(dataframe["close"], timeperiod=20) - 0.05 * ta.SMA(
            dataframe["close"], timeperiod=20
        )
        dataframe["hclose"] = (
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            ((dataframe["high"] < dataframe["smad1"]) & (dataframe["hopen"] < dataframe["hclose"])),
            "enter_long",
        ] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            ((dataframe["low"] > dataframe["smau1"]) & (dataframe["hopen"] > dataframe["hclose"])),
            "exit_long",
        ] = 1
        return dataframe
