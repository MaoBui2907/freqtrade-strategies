...
class cryptohassle(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ha_macd_cross_above"] = qtpylib.crossed_above(
            dataframe["ha_macd"], dataframe["ha_macdsignal"]
        )
        ...
        def SSLChannels(dataframe, length=10, mode="sma"):
            ...
            df["hlv"] = np.where(
                df["ha_close"] > df["smaHigh"],
                1,
                np.where(df["ha_close"] < df["smaLow"], -1, np.NAN),
            )
            df["hlv"] = df["hlv"].ffill()
        ...
        dataframe["ha_ssl_cross_above"] = qtpylib.crossed_above(
            dataframe["ha_sslUp"], dataframe["ha_sslDown"]
        )
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                # Heikin Ashi SSL Channels
                (
                    dataframe["ha_ssl_cross_above"].rolling(5).apply(lambda x: x.any(), raw=False)
                    == 1
                )
                &
                # Momentum
                (
                    dataframe["ha_mom_cross_above"].rolling(5).apply(lambda x: x.any(), raw=False)
                    == 1
                )
                &
                # Heikin Ashi MacD
                (
                    dataframe["ha_macd_cross_above"].rolling(5).apply(lambda x: x.any(), raw=False)
                    == 1
                )
                &
                # Volume
