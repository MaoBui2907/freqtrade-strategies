...
class LuxOSC(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["osc"], dataframe["signal"], dataframe["histogram"], dataframe["supertrend"] = (
            LUX_SuperTrendOscillator(
                dataframe,
                length=int(self.length_buy.value),
                mult=int(self.mult_buy.value),
                smooth=int(self.smooth_buy.value),
            )
        )
        return dataframe
