...
class Schism3(IStrategy):
    ...
    sell_params = {"rmi-high": 50, "rmi-low": 10}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["bounce-price"] = np.where(
            ...
            dataframe["close"],
            getattr(
                dataframe["close"].rolling(params["bounce-lookback"], min_periods=1),
                params["bounce-price"],
            )(),
        )

        # Count how many times our pre-bounce happned in the last "lookback" candles
        dataframe["bounce-range"] = np.where(
            dataframe["bounce-pending"].rolling(params["bounce-lookback"], min_periods=1).sum()
            >= 1,
            1,
            0,
        )
        ...
        # Normal buy triggers that apply to new trades we want to enter
            ...
            if self.config["stake_currency"] in ("BTC", "ETH"):
                ...
                conditions.append(
                    dataframe[f"{self.config['stake_currency']}_rmi_{self.inf_timeframe}"]
