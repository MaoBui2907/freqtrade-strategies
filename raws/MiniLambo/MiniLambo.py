...
class MiniLambo(IStrategy):
    ...
    protection_cooldown_period = IntParameter(
        low=1, high=48, default=1, space="protection", optimize=True
    )

    protection_maxdrawdown_lookback_period_candles = IntParameter(
        low=1, high=48, default=1, space="protection", optimize=True
    )
    protection_maxdrawdown_trade_limit = IntParameter(
        low=1, high=8, default=4, space="protection", optimize=True
    )
    protection_maxdrawdown_stop_duration_candles = IntParameter(
        low=1, high=48, default=1, space="protection", optimize=True
    )
    protection_maxdrawdown_max_allowed_drawdown = DecimalParameter(
        low=0.01, high=0.20, default=0.1, space="protection", optimize=True
    )

    protection_stoplossguard_lookback_period_candles = IntParameter(
        low=1, high=48, default=1, space="protection", optimize=True
    )
    protection_stoplossguard_trade_limit = IntParameter(
        low=1, high=8, default=4, space="protection", optimize=True
    )
    protection_stoplossguard_stop_duration_candles = IntParameter(
        low=1, high=48, default=1, space="protection", optimize=True
    )
    ...
    lambo2_rsi_4_limit = IntParameter(
        5, 60, default=buy_params["lambo2_rsi_4_limit"], space="entry", optimize=False
    )
    lambo2_rsi_14_limit = IntParameter(
        5, 60, default=buy_params["lambo2_rsi_14_limit"], space="entry", optimize=False
    )
    lambo2_rsi_21_limit = IntParameter(
        5, 60, default=buy_params["lambo2_rsi_21_limit"], space="entry", optimize=False
    )

    lambo2_pct_change_low_period = IntParameter(
        1, 60, default=buy_params["lambo2_pct_change_low_period"], space="entry", optimize=True
    )
    lambo2_pct_change_low_ratio = DecimalParameter(
        low=-0.20,
        high=-0.01,
        decimals=3,
        default=buy_params["lambo2_pct_change_low_ratio"],
        space="entry",
        optimize=True,
    )

    lambo2_pct_change_high_period = IntParameter(
        1, 180, default=buy_params["lambo2_pct_change_high_period"], space="entry", optimize=True
    )
    lambo2_pct_change_high_ratio = DecimalParameter(
        low=-0.30,
        high=-0.01,
        decimals=3,
        default=buy_params["lambo2_pct_change_high_ratio"],
        space="entry",
        optimize=True,
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        lambo2 = (
            ...
            & (
                dataframe["close"].pct_change(periods=self.lambo2_pct_change_low_period.value)
                < float(self.lambo2_pct_change_low_ratio.value)
            )
            & (
                dataframe["close"].pct_change(periods=self.lambo2_pct_change_high_period.value)
                > float(self.lambo2_pct_change_high_ratio.value)
            )
        )
