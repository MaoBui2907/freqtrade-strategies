...
class NotAnotherSMAOffsetStrategyLite(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        for length in set(
            list(self.base_nb_candles_buy.range) + list(self.base_nb_candles_sell.range)
        ):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                (
                    dataframe["close"]
                    < (dataframe[f"ema_{self.base_nb_candles_buy.value}"] * self.low_offset.value)
                )
                & (dataframe["ewo"] > 0)
                & (dataframe["volume"] > 0)
            ),
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                (
                    dataframe["close"]
                    > (dataframe[f"ema_{self.base_nb_candles_sell.value}"] * self.high_offset.value)
                )
                & (dataframe["volume"] > 0)
            ),
            "exit_long",
