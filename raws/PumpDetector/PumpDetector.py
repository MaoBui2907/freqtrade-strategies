...
class PumpDetector(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["var1_max"] = np.where(
            (dataframe["low"] - dataframe["var1"]) > 0, (dataframe["low"] - dataframe["var1"]), 0
        )
        dataframe["var2_test"] = xsa(dataframe, source="var1_abs", len=3, wei=1)
        dataframe["var2"] = (
            xsa(dataframe, source="var1_abs", len=3, wei=1)
            / xsa(dataframe, source="var1_max", len=3, wei=1)
        ) * 100
        ...
        dataframe["var6"] = 1
        dataframe["var7_data"] = np.where(
            dataframe["low"] <= dataframe["var4"],
            (dataframe["var3"] + dataframe["var5"] * 2) / 2,
            0,
        )
        dataframe["var7"] = ta.EMA(dataframe["var7_data"], timeperiod=3) / 618 * dataframe["var3"]

        dataframe["var8"] = (
            (dataframe["close"] - dataframe["low"].rolling(21).min())
            / (dataframe["high"].rolling(21).max() - dataframe["low"].rolling(21).min())
        ) * 100
        dataframe["var9"] = xsa(dataframe, source="var8", len=13, wei=8)

        dataframe["rsv"] = (
            (dataframe["close"] - dataframe["low"].rolling(n1).min())
            / (dataframe["high"].rolling(n1).max() - dataframe["low"].rolling(n1).min())
            * 100
        )
