...
class ObeliskRSI_v6_1(IStrategy):
    ...
    buy_params = {"bear-buy-rsi-value": 21, "bull-buy-rsi-value": 35}

    # Sell hyperspace params:
    sell_params = {"bear-sell-rsi-value": 55, "bull-sell-rsi-value": 69}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            (
                (dataframe["bull"] > 0)
                & qtpylib.crossed_below(dataframe["rsi"], params["bull-buy-rsi-value"])
            )
            | (
                ~(dataframe["bull"] > 0)
                & qtpylib.crossed_below(dataframe["rsi"], params["bear-buy-rsi-value"])
            )
        )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        sl_pct = 1 - easeInCubic(
            clamp01(since_open / timedelta(minutes=self.custom_stop_ramp_minutes))
        )
