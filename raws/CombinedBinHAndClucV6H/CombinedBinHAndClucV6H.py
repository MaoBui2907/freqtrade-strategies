...
class CombinedBinHAndClucV6H(IStrategy):
    ...
    buy_cluc_volume = IntParameter(
        10, 40, default=21, space="entry", optimize=cust_optimize["buy_cluc_volume"], load=True
    )
    ...
    buy_minimum_conditions = IntParameter(
        1, 2, default=1, space="entry", optimize=cust_optimize["buy_minimum_conditions"], load=True
    )
    ...
    cstp_bail_time = IntParameter(
        1, 36, default=5, space="exit", optimize=cust_optimize["cstp_bail_time"]
    )
    cstp_loss_threshold = DecimalParameter(
        -0.25, 0, default=0, decimals=2, space="exit", optimize=cust_optimize["cstp_loss_threshold"]
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if self.config["runmode"].value in ("backtest", "hyperopt"):
            assert (
                timeframe_to_minutes(self.timeframe) <= 5
            ), "Backtest this strategy in a timeframe of 5m or less."
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                & dataframe["closedelta"].gt(
                    dataframe["close"] * self.buy_bin_closedelta_close.value
                )
        ...
        dataframe.loc[
            (
                ...
                & (
                    dataframe["close"]
                    < self.buy_cluc_close_bblowerband.value * dataframe["bb_lowerband"]
                )
                & (
                    dataframe["volume"]
                    < (dataframe["volume_mean_slow"].shift(1) * self.buy_cluc_volume.value)
                )
        ...
        dataframe.loc[:, "conditions_count"] = (
            dataframe["buy_cond_bin"].astype(int)
            + dataframe["buy_cond_cluc"].astype(int)
            + dataframe["buy_cond_long"].astype(int)
        )
