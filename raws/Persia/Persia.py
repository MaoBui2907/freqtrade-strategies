...
class Persia(IStrategy):
    ...
    timeframe0 = (
        CategoricalParameter(
            timeframes, default=timeframes[0], optimize=0 < CONDITIONS, space="entry"
        )
        if COSTUMETFENABLED
        else IntParameter(
            timeframes[0],
    ...
    sell_timeframe0 = (
        CategoricalParameter(
            timeframes, default=timeframes[0], optimize=0 < CONDITIONS, space="exit"
        )
        if COSTUMETFENABLED
        else IntParameter(
            timeframes[0],
            ...
            for tf_idx in timeframes:
                ...
                try:
                    dataframe[f"{indicator}-{tf_idx}"] = getattr(ta, indicator)(
                        dataframe1, timeperiod=tf_idx
                    )
                except:
                    try:
                        dataframe[f"{indicator}-{tf_idx}"] = getattr(ta, indicator)(
                            dataframe1, timeperiod=float(tf_idx)
                        )
                    except:
                        try:
                            dataframe[f"{indicator}-{tf_idx}"] = getattr(ta, indicator)(
