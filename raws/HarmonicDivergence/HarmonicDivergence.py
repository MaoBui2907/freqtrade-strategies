    ...
    def add_total_divergences_in_config(self, dataframe):
        ...
        self.config["main_plot"][resample("total_bullish_divergences")] = {
            "plotly": {
                ...
                "marker": {"symbol": "diamond", "size": 11, "line": {"width": 2}, "color": "green"},
            }
        }
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        for i in range(1, len(dataframe["close"])):
            if (
                dataframe.iloc[-i]["date"].to_pydatetime().replace(tzinfo=datetime.timezone.utc)
                == trade.open_date_utc
            ):
...
def ema_cross_check(dataframe):
    ...
    return ~(
        dataframe["ema20_50_cross"] | dataframe["ema20_200_cross"] | dataframe["ema50_200_cross"]
    )
    ...
    if high_iterator[index] == index:
        ...
        for i in range(current_index - 1, current_index - 6, -1):
            ...
            if np.isnan(prev_pivot):
                return
            if (
                dataframe["pivot_highs"][current_pivot] < dataframe["pivot_highs"][prev_pivot]
    ...
    if low_iterator[index] == index:
        ...
        for i in range(current_index - 1, current_index - 6, -1):
            ...
            if np.isnan(prev_pivot):
                return
            if (
                dataframe["pivot_lows"][current_pivot] < dataframe["pivot_lows"][prev_pivot]
                and indicator[current_pivot] > indicator[prev_pivot]
            ) or (
                dataframe["pivot_lows"][current_pivot] > dataframe["pivot_lows"][prev_pivot]
                and indicator[current_pivot] < indicator[prev_pivot]
