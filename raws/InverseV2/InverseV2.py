...
class InverseV2(IStrategy):
    ...
    buy_fisher_cci_1 = DecimalParameter(
        low=-0.6, high=-0.3, decimals=2, default=-0.5, space="entry", optimize=True, load=True
    )
    buy_fisher_cci_2 = DecimalParameter(
        low=0.3, high=0.6, decimals=2, default=0.5, space="entry", optimize=True, load=True
    )

    sell_fisher_cci_1 = DecimalParameter(
        low=0.3, high=0.6, decimals=2, default=0.5, space="exit", optimize=True, load=True
    )
    sell_fisher_cci_2 = DecimalParameter(
        low=-0.6, high=-0.3, decimals=2, default=-0.5, space="exit", optimize=True, load=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.info_timeframe != "none":
            ...
            dataframe = merge_informative_pair(
                dataframe, informative_p, self.timeframe, self.info_timeframe, ffill=True
            )
        ...
        if self.info_timeframe != "none":
            ...
            dataframe = merge_informative_pair(
                dataframe, informative_btc, self.timeframe, self.info_timeframe, ffill=True
            )
            drop_columns = [(s + "_" + self.info_timeframe) for s in ["date"]]
            dataframe.drop(columns=dataframe.columns.intersection(drop_columns), inplace=True)
